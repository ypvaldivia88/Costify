...
import { buildDemoWorkspace, DEMO, DEMO_IDS } from './seed-demo-tenant';
...
function enhanceWorkspaceForMultilocal(
  ...
  const merged = migrateWorkspaceLocations(
    {
      ...workspace,
      locations,
      warehouses,
...
async function main() {
  ...
  if (!demo) {
    console.error(
      `No existe el usuario ${DEMO.adminEmail}. Ejecuta primero: pnpm seed:demo`
    );
    process.exit(1);
  }
  ...
  console.log(`  ${formatSubscriptionLocationBreakdown(subscription.locationCount)}`);
  console.log(`  ${subscription.monthlyPriceUsd} USD/mes (${subscription.plan})`);
  console.log('');
  console.log('Datos para probar conciliación (julio 2026):');
  console.log('  CENTRO: vendidos 15 refrescos vs salida inventario 12 → alerta');
  console.log('  PLAYA: pan sobao cuadra (20); pastel vendidos 5 vs salida 3 → alerta');
  console.log('');
  console.log('SKUs POS en productos: REFRESCO_355, PAN_SOBAO, PASTEL_QUESO');
  console.log('Plantilla CSV: date,location_code,sku,quantity,unit_price');
